...
def generate_mmr_graph(tool_context: ToolContext, gamemode: str, limit_seasons: int = None):
    """
    ...
    sorted_points = sorted(zip(seasons, mmr_values))
    
    # Filter by last N seasons if specified
    if limit_seasons and isinstance(limit_seasons, int) and limit_seasons > 0:
        sorted_points = sorted_points[-limit_seasons:]
        
    seasons = [pt[0] for pt in sorted_points]
...
def compare_players_graph(tool_context: ToolContext, player1_id: str, player1_platform: str, player2_id: str, player2_platform: str, gamemode: str, limit_seasons: int = None):
    """
